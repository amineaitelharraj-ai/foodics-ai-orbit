/**
 * Labeeb WebSocket Client
 * Pure TypeScript WebSocket client for communication with the Labeeb AI backend
 */
...
export interface LabeebClientOptions {
  ...
  onQueryClarification?: (data: QueryClarificationData) => void;
  ...
  onError?: (error: LabeebError) => void;
...
function createLabeebError(message: string, code?: number, pendingTool?: string): LabeebError {
  const error = new Error(message) as LabeebError;
  if (code !== undefined) error.code = code;
  if (pendingTool !== undefined) error.pendingTool = pendingTool;
  return error;
}
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        console.log(
          ...
          urlWithAuth.replace(/token=.*/, 'token=[REDACTED]')
        );
      ...
      this.ws.onerror = () => {
        ...
        this.options.onError?.(createLabeebError('WebSocket error'));
      ...
      this.ws.onclose = (event: CloseEvent) => {
        ...
        if (errorMessage) {
          const error = createLabeebError(errorMessage, event.code);
    ...
    switch (data.type) {
      case 'token': {
        ...
        if (typeof data.content === 'string') {
          ...
          const contentObj = data.content as { text?: string };
          text = contentObj.text ?? JSON.stringify(data.content);
        } else {
      ...
      case 'refinement_applied':
        ...
        break;
      case 'error': {
        const error = createLabeebError(data.message, undefined, data.pending_tool);
        this.options.onError?.(error);
        break;
      }
      case 'pong':
